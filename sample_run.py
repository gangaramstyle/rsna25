...
def _(
    ...
    def train_model():
        ...
        TRANSFORMER_DEPTH = 16     # Number of transformer blocks
        ...
        BATCH_SIZE = 128
        LEARNING_RATE = 1e-4
        TRAINING_STEPS = 25000 # Since the dataset is infinite, we train for a fixed number of steps
        ...
        # --- Setup Logging ---
        log_filename = f"training_log_BIG_MODEL_{int(time.time())}.csv"
        ...
        for step in range(TRAINING_STEPS):
            try:
                ...
                loss_scan = supervised_contrastive_loss(fused_scan_cls, fused_scan_y)

                fused_pos_cls = torch.cat((x1[:, 1], x2[:, 1]), dim = 1)
                pos_predictions = model.trunk(fused_pos_cls)
                loss_pos = position_criterion(pos_predictions, y)

                total_loss = (1.0 * loss_scan) + (5.0 * loss_pos)
                ...
                if (step + 1) % 10 == 0:
                    ...
                    loss_pos_item = loss_pos.item()
                    loss_scan_item = loss_scan.item()
                    ...
                    print(f"Step [{step+1}/{TRAINING_STEPS}], "
                          ...
                          f"Scan Loss: {loss_scan_item:.4f}, "
                          f"Pos Loss: {loss_pos_item:.4f}")

                    # Log all three loss values for better analysis later
                    with open(log_filename, 'a') as log_file:
                        log_file.write(f"{step+1},{timestamp},{total_loss_item:.4f},{loss_scan_item:.4f},{loss_pos_item:.4f}\n")
        ...
        final_model_save_path = os.path.join(OUTPUT_DIR, f"final_big_model_step_{TRAINING_STEPS}.pth")
